...
export const addProductSchema = z.object({
    title: z.string({ required_error: "Це поле є обов'язковим" }).min(1),
    price: z.string().min(1, "Це поле є обов'язковим"),
    description: z.string({ required_error: "Це поле є обов'язковим" }).min(1),
    category: z.string()
})

export const userAuthSchema = z.object({
    username: z
        .string({ required_error: 'Це поле не можу пустувати' })
        .min(4, 'Введіть щонайменше 4 символи'),
    password: z
